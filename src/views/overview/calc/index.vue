...
<script>
import { CalcInfo, CalcChart } from '@/api/overview.calc'
...
export default {
  ...
  data () {
    return {
      chartSettings: {
        dimension: ['queueName'],
        metrics: ['availableMB', 'appsRunning']
      },
      chartData: {
  ...
  mounted () {
    ...
    CalcChart()
      .then(res => {
        ...
        // res.rows.map((item, index, array) => {
        //   item.createTime = publicMethod.getTimestamp(item.createTime)
        // })
